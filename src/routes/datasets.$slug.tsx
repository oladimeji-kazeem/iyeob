...
import { ArrowLeft, Bookmark, BookmarkCheck, Copy, Database, Download, FileJson, Search, ShieldCheck } from "lucide-react";
...
import { citationText, downloadDatasetSample } from "@/lib/download";
import { datasetBySlug } from "@/lib/datasets";
import { usePublishedDataset } from "@/lib/published";

export const Route = createFileRoute("/datasets/$slug")({
  head: ({ params }) => {
    const seeded = datasetBySlug(params.slug);
    const title = seeded ? `${seeded.title} Synthetic Dataset | IYEOB` : "Synthetic dataset | IYEOB";
    const description = seeded?.description ?? "A documented synthetic dataset published in the IYEOB repository.";
...
function DatasetDetailPage() {
  ...
  const { dataset, isLoading } = usePublishedDataset(slug);
  const { user } = useAuth();
  const { isBookmarked, toggle } = useBookmarks();
  const [columnQuery, setColumnQuery] = useState("");
  const [selected, setSelected] = useState<string[]>([]);

  const previewColumns = useMemo(() => Object.keys(dataset?.preview[0] ?? {}), [dataset]);

  useEffect(() => { setSelected(previewColumns); }, [previewColumns.join("|")]);
  useEffect(() => { if (dataset) trackEvent({ event_type: "dataset_view", dataset_slug: dataset.slug }); }, [dataset?.slug]);

  if (isLoading && !dataset) {
    return <AppLayout><div className="mx-auto max-w-3xl px-5 py-28 text-center text-sm text-muted-foreground">Loading dataset…</div></AppLayout>;
  }
  if (!dataset) {
    return <AppLayout><div className="mx-auto max-w-3xl px-5 py-28 text-center"><h1 className="font-display text-4xl font-bold">Dataset unavailable</h1><p className="mt-4 text-muted-foreground">This release may have moved or is no longer published.</p><Button className="mt-7" asChild><Link to="/datasets">Browse datasets</Link></Button></div></AppLayout>;
  }

  const columns = dataset.columns.filter((column) => `${column.name} ${column.description} ${column.type}`.toLowerCase().includes(columnQuery.toLowerCase()));
  const citation = citationText(dataset);

  const download = (format: "csv" | "json") => {
    downloadDatasetSample(dataset, format, selected);
    trackEvent({ event_type: "download", dataset_slug: dataset.slug, file_format: format, filters: { columns: String(selected.length) } });
    toast.success(`${format.toUpperCase()} sample downloaded with citation metadata`);
  };

  const toggleColumn = (name: string) =>
    setSelected((prev) => (prev.includes(name) ? prev.filter((column) => column !== name) : [...prev, name]));

  const copyCitation = async () => { await navigator.clipboard.writeText(citation); toast.success("Citation copied"); };

  return <AppLayout>
    <section className="border-b border-border bg-card"><div className="mx-auto max-w-7xl px-5 py-10 lg:px-8"><Link to="/datasets" className="inline-flex items-center gap-2 text-sm font-semibold text-muted-foreground hover:text-primary"><ArrowLeft className="size-4"/>All datasets</Link><div className="mt-8 flex flex-col justify-between gap-8 lg:flex-row lg:items-end"><div className="max-w-3xl"><div className="flex flex-wrap items-center gap-2"><span className="inline-flex items-center gap-1.5 rounded-full bg-synthetic px-2.5 py-1 text-[11px] font-bold uppercase tracking-wider text-synthetic-foreground"><ShieldCheck className="size-3.5"/>Synthetic data</span><span className="text-xs font-bold uppercase tracking-wider text-primary">{dataset.domain}</span></div><h1 className="mt-5 font-display text-4xl font-extrabold sm:text-5xl">{dataset.title}</h1><p className="mt-5 text-lg leading-8 text-muted-foreground">{dataset.description}</p><div className="mt-6 flex flex-wrap gap-x-6 gap-y-2 text-sm text-muted-foreground"><span>{dataset.country}</span><span>{dataset.task}</span><span>{dataset.difficulty}</span><span>Version {dataset.version}</span><span>{dataset.license}</span></div></div><div className="flex gap-2">{user && <Button variant="outline" aria-label={isBookmarked(dataset.slug) ? "Remove bookmark" : "Bookmark dataset"} onClick={() => toggle.mutate({ slug: dataset.slug, title: dataset.title }, { onSuccess: (result) => toast.success(result === "added" ? "Bookmarked" : "Bookmark removed"), onError: (error) => toast.error(error instanceof Error ? error.message : "Could not update bookmark") })}>{isBookmarked(dataset.slug) ? <BookmarkCheck className="text-primary"/> : <Bookmark/>}{isBookmarked(dataset.slug) ? "Bookmarked" : "Bookmark"}</Button>}<Button variant="outline" onClick={() => download("json")}><FileJson/>JSON</Button><Button onClick={() => download("csv")}><Download/>Download CSV</Button></div></div><p className="mt-4 text-xs text-muted-foreground">Every download includes a separate citation metadata file and covers the {selected.length} column{selected.length === 1 ? "" : "s"} you selected.</p></div></section>
    ...
    <section className="mx-auto max-w-7xl px-5 py-12 lg:px-8"><div className="grid gap-10 lg:grid-cols-[1fr_320px]"><div><Tabs defaultValue="overview"><TabsList className="h-auto w-full justify-start overflow-x-auto border-b border-border bg-transparent p-0"><Tab value="overview">Overview</Tab><Tab value="dictionary">Data dictionary</Tab><Tab value="preview">Preview</Tab><Tab value="methodology">Methodology</Tab></TabsList>
      <TabsContent value="overview" className="pt-8"><Section title="What this dataset represents"><p>{dataset.description}</p><h3>Intended use</h3><p>{dataset.intendedUse}</p><h3>Ethical considerations</h3><p>Use this dataset for learning, prototyping, and research. Do not use synthetic outputs to infer facts about individuals, communities, companies, or Nigerian population statistics.</p></Section><div className="mt-9 grid gap-4 sm:grid-cols-3">{[["Rows",dataset.rows.toLocaleString()],["Variables",dataset.columns.length],["Size",dataset.size]].map(([label,value])=><div key={label} className="border border-border bg-card p-5"><span className="text-xs font-bold uppercase tracking-wider text-muted-foreground">{label}</span><div className="mt-2 font-display text-2xl font-extrabold">{value}</div></div>)}</div>{dataset.researchQuestions.length > 0 && <Section title="Research questions"> <ul>{dataset.researchQuestions.map((item)=><li key={item}>{item}</li>)}</ul></Section>}</TabsContent>
      <TabsContent value="dictionary" className="pt-8"><div className="relative mb-5"><Search className="absolute left-3 top-1/2 size-4 -translate-y-1/2 text-muted-foreground"/><Input value={columnQuery} onChange={(e)=>setColumnQuery(e.target.value)} placeholder="Search variables" className="pl-10"/></div><div className="overflow-x-auto border border-border"><table className="w-full min-w-[720px] text-left text-sm"><thead className="bg-muted text-xs uppercase text-muted-foreground"><tr>{["Include","Variable","Type","Description","Example","Role"].map((x)=><th className="px-4 py-3" key={x}>{x}</th>)}</tr></thead><tbody>{columns.map((c)=><tr key={c.name} className="border-t border-border"><td className="px-4 py-4">{previewColumns.includes(c.name) ? <Checkbox checked={selected.includes(c.name)} onCheckedChange={()=>toggleColumn(c.name)} aria-label={`Include ${c.name} in downloads`}/> : <span className="text-xs text-muted-foreground">—</span>}</td><td className="px-4 py-4 font-mono font-semibold text-primary">{c.name}</td><td className="px-4 py-4">{c.type}</td><td className="px-4 py-4 text-muted-foreground">{c.description}</td><td className="px-4 py-4 font-mono">{c.example}</td><td className="px-4 py-4">{c.role}</td></tr>)}</tbody></table></div><p className="mt-3 text-xs text-muted-foreground">Selected columns are the ones written into your CSV or JSON download.</p></TabsContent>
      <TabsContent value="preview" className="pt-8"><p className="mb-5 text-sm text-muted-foreground">A small sample from this synthetic release.</p><div className="overflow-x-auto border border-border"><table className="w-full min-w-max text-left text-sm"><thead className="bg-muted"><tr>{previewColumns.map((key)=><th className="px-4 py-3 font-mono text-xs" key={key}>{key}</th>)}</tr></thead><tbody>{dataset.preview.map((row,index)=><tr key={index} className="border-t border-border">{previewColumns.map((key)=><td className="px-4 py-4" key={key}>{String(row[key] ?? "")}</td>)}</tr>)}</tbody></table></div></TabsContent>
